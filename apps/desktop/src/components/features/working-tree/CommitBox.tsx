  ...
  return (
    <div className="px-3 py-3 border-t border-border-60 bg-surface-1-10 space-y-2.5 shrink-0">
      <div className="commit-box-shell flex flex-col bg-surface-2-30 border border-border-40 rounded-mac p-2.5 transition-all shadow-2xs focus-within:border-accent-40 focus-within:shadow-accent-5">
        ...
        {amend && (
          <div className="flex items-center gap-1.5 px-1 py-0.5 mb-1 rounded text-[9px] font-semibold text-[#ff9f0a] bg-[#ff9f0a]/10 border border-[#ff9f0a]/25 select-none">
            <GitCommit size={9} className="shrink-0" />
            <span>Amending — this message will replace the last commit's message{staged.length > 0 ? ` (${staged.length} staged file${staged.length === 1 ? "" : "s"} will be folded in)` : " (no file changes)"}</span>
          </div>
        )}
        ...
        <textarea
          ...
          placeholder={amend ? "New message for last commit…" : "Commit message"}
