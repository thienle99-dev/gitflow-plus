...
export default function UpdateChecker({ onClose, autoCheck }: UpdateCheckerProps) {
  ...
  const check = useCallback(async () => {
    ...
    try {
      ...
      if (!update) { setStatus("latest"); return; }
  ...
  const close = () => { if (status !== "downloading") onClose?.(); };
      ...
      <div className="space-y-3 min-h-[100px] flex flex-col justify-center">
        ...
        {status === "checking" && (
          <div className="flex items-center gap-2 text-xs text-text-secondary justify-center py-4">
            <Loader2 size={14} className="animate-spin text-accent" /> Checking for updates...
          </div>
        )}
        ...
        {status === "downloading" && (
          <div className="space-y-3 py-2">
            <div className="flex items-center gap-2 text-xs text-text-secondary">
              <Loader2 size={13} className="animate-spin text-accent" /> Downloading... {progress}%
            </div>
